...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $content = "";
    foreach ($ptc_cfg as $key => $val) { $content .= "$key=\""; $content .= $val; $content .= "\"
"; }
...
.form-pair label { flex: 0 0 110px; color: var(--primary-blue); font-weight: bold; font-size: 13px; position: relative; cursor: help; }
...
.ptc-input { background: #111 !important; border: 1px solid #444 !important; border-radius: 4px !important; color: #fff !important; padding: 5px 8px !important; width: 100% !important; box-sizing: border-box !important; font-size: 12.5px !important; height: 30px !important; }
...
.btn-ptc { background: transparent; color: var(--primary-blue); border: 1px solid var(--primary-blue); border-radius: 4px; cursor: pointer; font-weight: bold; transition: background 0.1s; }
.btn-ptc:hover { background: rgba(0, 170, 255, 0.15); }

.btn-save { padding: 10px 15px; font-size: 13px; width: 100%; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px; }
.btn-add { padding: 6px 12px; font-size: 11px; margin-top: 10px; }

#ptc-log { background: #000; border: 1px solid #333; border-radius: 8px; color: #00ffaa; font-family: 'Courier New', monospace; font-size: 12.5px; padding: 12px; flex-grow: 1; overflow-y: auto; white-space: pre-wrap; word-break: break-all; margin-top: 10px; height: 100%; min-height: 500px; }
...
<form method="post" autocomplete="off">
    <div id="ptc-wrapper">
        ...
        <div class="ptc-col" id="ptc-col-servers">
            ...
            <div class="form-pair"><label data-tooltip="URL of your Plex Server (e.g. http://192.168.1.100:32400).">URL:</label><div class="form-input-wrapper"><input type="text" name="PLEX_URL" value="<?= $ptc_cfg['PLEX_URL'] ?>" class="ptc-input"></div></div>
            <div class="form-pair"><label data-tooltip="Your Plex X-Plex-Token.">Token:</label><div class="form-input-wrapper"><input type="password" name="PLEX_TOKEN" value="<?= $ptc_cfg['PLEX_TOKEN'] ?>" class="ptc-input" onmouseover="this.type='text'" onmouseout="this.type='password'" autocomplete="new-password"></div></div>

            <div class="section-header"><i class="fa fa-server"></i> Emby Server</div>
            <div class="form-pair"><label data-tooltip="Check to enable Emby stream monitoring.">Enable:</label><div class="form-input-wrapper"><input type="checkbox" name="ENABLE_EMBY" value="True" <?= $ptc_cfg['ENABLE_EMBY'] == 'True' ? 'checked' : '' ?> ></div></div>
            ...
            <div class="form-pair"><label data-tooltip="Check to enable Jellyfin stream monitoring.">Enable:</label><div class="form-input-wrapper"><input type="checkbox" name="ENABLE_JELLYFIN" value="True" <?= $ptc_cfg['ENABLE_JELLYFIN'] == 'True' ? 'checked' : '' ?> ></div></div>
        ...
        <div class="ptc-col" id="ptc-col-tuning">
            ...
            <div class="form-pair"><label data-tooltip="The root directory of your Unraid array (usually /mnt/user).">Array Root:</label><div class="form-input-wrapper"><input type="text" name="ARRAY_ROOT" value="<?= $ptc_cfg['ARRAY_ROOT'] ?>" class="ptc-input"></div></div>
            <div class="form-pair"><label data-tooltip="The root directory of your cache pool (e.g. /mnt/cache).">Cache Root:</label><div class="form-input-wrapper"><input type="text" name="CACHE_ROOT" value="<?= $ptc_cfg['CACHE_ROOT'] ?>" class="ptc-input"></div></div>
            <div class="form-pair"><label data-tooltip="Directory names to ignore (comma separated).">Exclude:</label><div class="form-input-wrapper"><input type="text" name="EXCLUDE_DIRS" value="<?= $ptc_cfg['EXCLUDE_DIRS'] ?>" placeholder="temp,skip" class="ptc-input"></div></div>
            ...
            <div class="form-pair"><label data-tooltip="Seconds between session checks.">Interval:</label><div class="form-input-wrapper"><input type="number" name="CHECK_INTERVAL" value="<?= $ptc_cfg['CHECK_INTERVAL'] ?>" class="ptc-input input-small"><span class="unit-label">sec</span></div></div>
            <div class="form-pair"><label data-tooltip="Wait this many seconds after playback starts before moving file to cache.">Copy Delay:</label><div class="form-input-wrapper"><input type="number" name="COPY_DELAY" value="<?= $ptc_cfg['COPY_DELAY'] ?>" class="ptc-input input-small"><span class="unit-label">sec</span></div></div>
            <div class="form-pair"><label data-tooltip="Stop moving files if cache pool usage is above this percentage.">Max Cache:</label><div class="form-input-wrapper"><input type="number" name="CACHE_MAX_USAGE" value="<?= $ptc_cfg['CACHE_MAX_USAGE'] ?>" class="ptc-input input-small"><span class="unit-label">%</span></div></div>
            <div class="form-pair"><label data-tooltip="Automatically remove files from cache after they have been watched.">Smart Clean:</label><div class="form-input-wrapper"><input type="checkbox" name="ENABLE_SMART_CLEANUP" value="True" <?= $ptc_cfg['ENABLE_SMART_CLEANUP'] == 'True' ? 'checked' : '' ?> ></div></div>
            <div class="form-pair"><label data-tooltip="Seconds to wait after a movie is watched before removal from cache.">Del Delay:</label><div class="form-input-wrapper"><input type="number" name="MOVIE_DELETE_DELAY" value="<?= $ptc_cfg['MOVIE_DELETE_DELAY'] ?>" class="ptc-input input-small"><span class="unit-label">sec</span></div></div>
            <div class="form-pair"><label data-tooltip="Number of watched episodes to keep on cache.">Keep Prev:</label><div class="form-input-wrapper"><input type="number" name="EPISODE_KEEP_PREVIOUS" value="<?= $ptc_cfg['EPISODE_KEEP_PREVIOUS'] ?>" class="ptc-input input-small"><span class="unit-label">ep</span></div></div>
        </div>
