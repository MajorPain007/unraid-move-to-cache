...
define('PTC_SCAN_CAP', 20000);   // media files counted per folder before giving up
...
function ptc_media_under($dir, $cap = 0) {
    ...
    try {
        ...
        foreach ($it as $file) {
            ...
            if ($cap > 0 && count($out['files']) >= $cap) { $out['capped'] = true; break; }
        }
    } catch (Exception $e) {
...
if (isset($_GET['action']) && $_GET['action'] === 'browse') {
    ...
    // What is actually in the folder, not just what the plugin put there - the
    // point of the number is to tell a full folder from an empty one. Capped so
    // an enormous tree cannot stall the request; the UI says when it was.
    $summarise = function ($dir) use ($tracked, $streaming) {
        $under  = ptc_media_under($dir, PTC_SCAN_CAP);
        $cached = 0; $busy = 0;
...
if (isset($_GET['action']) && $_GET['action'] === 'uncache') {
    ...
    if (is_dir($path)) {
        ...
        if ($count === 0) {
            echo json_encode(['success' => false,
                'message' => 'No media files in ' . basename($path)]);
            exit;
        }
        $label = $count . ($under['capped'] ? '+' : '') . ' file(s) from ' . basename($path);
    } else {
        $label = basename($path);
    }
...
function ptcRow(body, o) {
    ...
    $('<td>').css({textAlign: 'right', color: '#98a5b2', whiteSpace: 'nowrap',
             ...
             .attr('title', o.noteTitle || o.note || '').text(o.note || '').appendTo(tr);
    ...
    $.post('/plugins/plex_to_cache/plex_to_cache.php?action=browse',
     ...
     .done(function(d) {
        ...
        d.dirs.forEach(function(dir) {
            ...
            if (!dir.files) {
                note = 'empty';
                full = 'No media files in this folder';
            } else {
                note = dir.cached + ' of ' + dir.files + (dir.capped ? '+' : '');
                full = dir.files + (dir.capped ? '+' : '') + ' media files, '
                     + dir.cached + ' on cache'
                     + (dir.busy ? ', ' + dir.busy + ' being streamed' : '');
            }
            ptcRow(body, {
                name: dir.name, icon: 'fa fa-folder', folder: true, title: dir.path,
                size: dir.size ? ptcBytes(dir.size) : '', note: note, noteTitle: full,
                open: function() { browseCache(dir.path); },
                move: dir.files ? function(btn) { uncacheFile(dir.path, btn); } : null,
                moveTitle: 'Move everything in this folder to the array. Anything being streamed stays.'
            });
        });
        ...
        d.files.forEach(function(f) {
            ptcRow(body, {
                ...
                size: ptcBytes(f.size), note: f.tracked ? 'cached' : '-',
                noteTitle: f.tracked ? 'This plugin put it on the cache'
                                     : 'On the cache, but not put there by this plugin',
